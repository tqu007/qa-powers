...
### 盘古业务全景对齐（Pangu Domain Alignment，资深业务专家模式，强制）
不仅机械提取字面文本，必须以**盘古资深业务专家**视角，深刻理解需求在系统全生命周期数据大动脉中的核心价值与商业逻辑（参考知识库：[盘古业务全景与技术知识库](https://confluence.rccchina.com/pages/viewpage.action?pageId=665608989)）：

#### 1. 盘古业务大动脉全生命周期流转对齐
- **阶段 1：公告处理 (`/bid`)**：外部公告 (bid-ms) ──▶ 规则分配过滤 ──▶ 查重关联 ──▶ 确认为高价值资讯；
- **阶段 2：资源跟进 (`/resource`)**：待跟进资源池 ──▶ 信息员抢占/认领 (Grab) ──▶ 电话深入调研；
- **阶段 3：项目加工 (`/project`)**：核实真实性 ──▶ 录入工程阶段、造价、地块等核心要素；
- **阶段 4：关系人穿透 (Contacts & Firms)**：穿透关联甲方/业主、设计院、施工总包、专业分包，沉淀关键决策人；
- **阶段 5：质量审批与质检**：主管审核 ──▶ 质检抽查 ──▶ 差错核对 ──▶ 构思递交上限校验；
- **阶段 6：外网发布与展现 (Reach)**：写入发布库 ──▶ 外网门户供付费客户检索；
- **阶段 7：采购促单与撮合 (`/pressing_order`)**：设备采购需求 ──▶ 库内供需撮合 ──▶ 协同 Reach 等待列表 (`/client_request/my_waiting_list`) 与挑战任务 (`/firm_task/task_list`)；
- **阶段 8：绩效核算与激励 (`/res_bonus_admin`)**：分期成绩 (Cutoff) ──▶ 团队大盘排名 ──▶ 绩优补助 ──▶ 年度海外游追踪。

#### 2. 战略级核心指标“大四重甲”与衍生判定矩阵
在涉及项目筛选、分期进阶考核或年度海外游的需求中，必须严格对照底层四维漏斗与判定规则设计用例：
- **大（大型项目）**：`major == 2`（投资额、建筑面积达到特大型/大型标准）；
- **四（四大重点行业）**：公用事业(18)、石油化工(11)、电力工程(22)及高端工业制造重点子类（医药、通信电子、高端装备、汽车等投资大宗）；
- **重（重点/完整优讯）**：完整对外发布优讯（`incomplete == 0 && version > 0`），严格排除不完整项目与临时资讯；
- **甲（甲方职能联系人）**：`Role::OWNER_PARTY_IDS`（业主、代建、政府机关、工艺师、采购负责人等出资与决策人）；
- **大四重甲 5 判定树 (`big_four_party_a5`)**：大四重项目中，**历史上首次实现甲方职能联系人达标 ≥ 5 个**，且本次新增中必须包含高价值关键人；
- **“小绿人”高价值联系人 (`big_four_high_value`)**：UI 带有绿色人形徽标的高管、设计总工、工艺师、采购总等核心决策人；
- **兄弟指标大酒医教甲**：大型酒店(2)、医疗(3)、教育科研(4)公建类项目的甲方决策人考核（如 `can_hotel_medical_edu_party_a4/a5_6`）；
- **业务不变量硬性门禁**：
  - 统计排查必须排除 `version = -3`（原始资讯）；
  - 分期先后比对严禁直接数值比对，必须使用 `Cutoff.compare_cutoff_id` 或 `ResBonus.after_xxxx` 进行补零比对。
